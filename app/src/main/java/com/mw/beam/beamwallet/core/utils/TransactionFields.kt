...
object TransactionFields {
    ...
    private const val SECOND_FIELD = "Second Amount"
    ...
    private const val ADDRESS_TYPE = "Address type"
    ...
    fun formatTransaction(txDescription: TxDescription): String {
        ...
        var addressType = txDescription.getAddressType(AppActivity.self)

        if (!txDescription.sender.value && (txDescription.isPublicOffline || txDescription.isShielded)) {
            addressType = AppActivity.self.getString(R.string.shielded_pool)
        }
        ...
        return "${if (txDescription.sender.value) SEND_BEAM_TYPE else RECEIVE_BEAM_TYPE} ," +
                ...
                "${txDescription.amount.convertToCurrencyString()} ," +
